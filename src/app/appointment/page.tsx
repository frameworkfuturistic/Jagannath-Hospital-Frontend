...
import { handleAppointmentAndPayment } from './payment-handler';
...
export default function Component() {
  ...
  const bookAppointment = async (data: any) => {
    ...
    try {
      ...
      if (!response.data.data.appointment?.OPDOnlineAppointmentID) {
        throw new Error('Server did not return a valid OPDOnlineAppointmentID');
      }
  ...
  const processPayment = async (data: { amount: number }) => {
    ...
    try {
      ...
      const appointmentId =
        state.appointmentDetails?.appointment?.OPDOnlineAppointmentID ?? null;
      console.log('Extracted OPDOnlineAppointmentID:', appointmentId);

      if (appointmentId === null) {
        throw new Error(
          'OPDOnlineAppointmentID is missing. Please book an appointment first.'
        );
      }
  ...
  const searchExistingPatient = useCallback(
    async (mrdOrMobile: string) => {
      try {
  ...
  const autoFillPatientData = useCallback(
    (patient: any) => {
      if (patient) {
      ...
      try {
        switch (currentStep) {
          ...
          case 1: // Patient Details
            ...
            setCurrentStep(currentStep + 1);
            break;
          case 2: // Payment and Appointment Creation
            ...
            const appointmentDetails = await handleAppointmentAndPayment(
              appointmentAndPaymentData,
              state,
              dispatch
            );
            dispatch({
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <div className="relative">
                      {' '}
                      {/* Added div for loading indicator */}
                      <CalendarComponent
                        mode="single"
                        selected={
                        ...
                        //disabledDates={daysWithoutSlots}
      ...
      <CardFooter className="bg-gray-50 p-6 flex justify-between">
        <div className="flex flex-col sm:flex-row justify-center gap-4">
          <Button asChild>
            <Link
              href="/"
              className="inline-flex items-center hover:bg-teal-600"
            >
      ...
      <CardContent className="p-6 space-y-6">
        <div className="space-y-4">
          ...
          {searchResults.length > 0 && (
            <Select
              onValueChange={(value) => autoFillPatientData(JSON.parse(value))}
            >
